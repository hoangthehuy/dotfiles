...
then
  ...
  venv_name="$(git rev-parse --show-toplevel 2> /dev/null)"
fi
potental_envs=("$HOME/.virtualenvs/$venv_name" "$HOME/src/$venv_name/.venv" "$HOME/src/$venv_name/.virtualenv")
for penv in $potental_envs
do
if [[ -d "$penv" ]]
then
...
source /home/discord/.nix-profile/etc/profile.d/nix.sh
